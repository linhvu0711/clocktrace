...
- Re-checked on macOS 27.0 (26A428) on 2026-09-22: same layout (`remote/<device-uuid>/` with 15-digit segment files of 512 KiB and a `tombstone/` subfolder holding older segments), same SEGB v2 layout (magic at offset 0, 32-byte header, 8-byte entry header with a CRC32, 16-byte trailer slots of end offset, state, timestamp; states 1 written, 3 deleted, 4 empty), same record fields (1, 3, 4, 6, 9, 10 as above; 9 and 10 absent in about half of the records; 2 and 13 are varints of unknown meaning). Source: the check script in PR #<n>, 2026-09-22.
