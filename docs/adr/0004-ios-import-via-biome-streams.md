...
Apple documents none of this and can move it in any release, so the importer lives in its own module that cannot break the Mac tracker, carries a list of macOS versions it was verified on and reports "not tested" on any other, exposes a health line (last event time, last import result) that turns into "iPhone import broken" after 48 hours without events or on a parse failure, and is covered by a golden test on one real sample file. Each June the Apple beta is installed on a second volume and the day-check script is rerun; v0 ships only after that check passes on macOS 26 and 27. This is the same yearly chore Timing carries (https://timingapp.com/help/screen-time).
