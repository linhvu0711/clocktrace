...
# --no-sign signs the app ad hoc, packs, and puts the version files back; CI
# runs it that way. Stops on the first failure.
set -euo pipefail
...
rm -rf "$stage" "$tarball"
pnpm --filter @clocktrace/cli deploy --prod "$stage"
